...
export function getWithdrawal(withdrawalRoot: Bytes): Withdrawal {
  let withdrawal = Withdrawal.load(withdrawalRoot)
  if(withdrawal == null) {
    withdrawal = new Withdrawal(withdrawalRoot);
    withdrawal.staker = Bytes.empty();
    withdrawal.nonce = BigInt.fromU32(0);
    withdrawal.startBlock = BigInt.fromU32(0);
    withdrawal.shares = [BigInt.fromU32(0)];
